    ...
    public class FilmCategoriesService
    {
        ...
        public async Task<FilmCategoriesDTO> CreateAsync(FilmCategoriesDTO newEntity)
        {
            try
            {
                ...
                {
                    Id = default,
                    FilmsId = newEntity.FilmId,
                    CategoriesId = newEntity.CategoryId
                };
        ...
        public async Task<FilmCategoriesDTO> UpdateAsync(FilmCategoriesDTO editEntity)
        {
            try
            {
                ...
                var categoryExists = await _categoryRepository.Get()
                    .AnyAsync(x => x.Id == editEntity.CategoryId);

                if (!categoryExists)
                    throw new InvalidIdException(ExceptionMessage(editEntity.CategoryId));

                var filmExists = await _filmRepository.Get()
                    .AnyAsync(x => x.Id == editEntity.FilmId);

                if (!filmExists)
                    throw new InvalidIdException(ExceptionMessage(editEntity.FilmId));

                currentEntity.FilmsId = editEntity.FilmId;
                currentEntity.CategoriesId = editEntity.CategoryId;
